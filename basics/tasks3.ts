...
const isProgram = (product: Course | Program): product is Program => {
...
const getPromotionMessage = (promotion: SpecificPromotion) /* add an explicit return type here  */ => {
    switch (promotion.scope) {
        ...
        case 'account':
            return `This account has a discount. Only valid with this listing ID: ${promotion.accountId}`
    }
}
...
const calculateDiscount = (promotion: Promotion): number => {
    ...
    let price = promotion.listingPrice;

    switch (promotion.type) {
        case 'flat':
            price = price - promotion.amount;
        case 'percentage':
            price = price - (price * promotion.amount)
    }
    if(price <= 0){
        return 0.0;
    }
